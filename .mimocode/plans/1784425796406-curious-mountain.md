...
## Goal
Finish the payment subgraph by fixing bugs, adding missing pieces, and aligning with the listing subgraph's DDD structure. No role renaming (patient stays patient, owner stays owner).
...
4. **Missing `paymentsByPatient` query resolver** — Schema defines it, no resolver implements it.
...
- Change constructor params from `(paymentId, bookingId, patientId, tenantId, amount, checkInDate, checkOutDate, refundedAt)` to `(paymentId, bookingId, patientId, tenantId, amount, completedAt)`
...
- Fix event construction: `new PaymentConfirmedEvent(payment.id, payment.bookingId, payment.patientId, payment.tenantId, payment.amount, payment.completedAt!)`
- Remove `console.log`
...
### Phase 5: Add `paymentsByPatient` query resolver + use case

**New file: `src/core/payment/application/usecase/getPaymentsByPatient.usecase.ts`**:
```typescript
@injectable()
...
- Add `getPaymentsByPatientUseCase: Symbol.for("GetPaymentsByPatientUseCase")`

**`src/modules/container/payment.register.ts`**:
- Register `GetPaymentsByPatientUseCase`

**`src/subgraphs/payment/payment.resolver.ts`**:
- Add `paymentsByPatient` resolver that resolves via the new use case
...
export class FailPaymentUseCase {
  ...
  async execute(paymentId: string) {
    ...
    await this.eventBus.publish(new PaymentFailedEvent(payment.id, payment.bookingId, payment.patientId, payment.tenantId, payment.amount));
...
| `src/core/payment/application/usecase/getPaymentsByPatient.usecase.ts` | **New** — use case for paymentsByPatient |
