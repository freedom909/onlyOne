...
import { PatientExtractor } from "./extractors/patient.extractor";
...
export class SemanticExtractor implements ISemanticExtractor {
  constructor(
    ...
    @inject(WISDOM_TOKENS.extractors.patientExtractor)
    private patientExtractor: PatientExtractor,
  ...
  async extract(request: WisdomRequest): Promise<SemanticContext> {
    ...
    const entities = [
      ...
      ...this.patientExtractor.extract(message),
    ];
