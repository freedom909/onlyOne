...
export class MessageRuleExtractor {
  ...
  extractEntities(message: string): SemanticEntity[] {
    return [
      ...
      ...this.extractPatientCount(message),
  ...
  /** "2 patients", "3 people" → CUSTOMER_COUNT */
  private extractPatientCount(message: string): SemanticEntity[] {
    const match = message.match(/(\d+)\s*(patients?|people|person|人)/i);
    if (!match) return [];
