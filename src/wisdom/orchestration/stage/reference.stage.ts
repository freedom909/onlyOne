...
export class ReferenceStage implements IPipelineStage {
  ...
  private populateResourcesFromSemantic(ctx: WisdomPipelineContext): void {
    ...
    // Patient count
    if (!resources.patientCount) {
      const patientEntity = entities.find(e => e.type === EntityType.CUSTOMER_COUNT);
      if (patientEntity) {
        resources.patientCount = parseInt(patientEntity.value as string, 10) || 1;
      }
    }
  }
}
