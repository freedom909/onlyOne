...
import { GetBookingsForPatientUseCase } from "@/core/booking/application/usecases/getBookingsForPatient.useCase";
...
export class BookingAgent implements IDomainAgent {
  constructor(
    ...
    @inject(delay(() => GetBookingsForPatientUseCase))
    private getBookingsForPatientUseCase: GetBookingsForPatientUseCase,
  ...
  private async handleCreateBooking(
    ...
    const patientCount = parseInt(this.extractEntity(semantic, ["CUSTOMER_COUNT", "patient_count", "guest_count"]) ?? "1");

    const patientId = context.identity.user?.id;
    if (!patientId) {
      return {
        success: false,
        domain: semantic.domain as any,
  ...
  private async handleGetMyBookings(context: AIContext): Promise<WisdomResponse> {
    ...
    const result = await this.getBookingsForPatientUseCase.execute(patientId);
    return {
      success: true,
      domain: "BOOKING" as any,
