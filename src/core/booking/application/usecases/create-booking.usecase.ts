...
async execute(input: CreateBookingInput, actor: BookingActor) {
  // 1. Validate — dates + listing come from input; patientId/tenantId from actor
  ...
  if (!actor?.patientId) {
    throw new Error("Missing required authentication: patientId");
  }
  ...
  try {
    ...
    const booking = Booking.create({
      ...
      tenantId: actor.tenantId || actor.patientId,
