// Rule 3: 清洁费规则
// CleaningFeeRuleService — calculates cleaning fee based on stay duration and patient count
...
const CLEANING_FEE_PER_patient = 500;       // ¥500 per extra patient above 2
...
export class CleaningFeeRuleService {
   ...
   *  - Additional fee per patient above 2
  ...
  static calculate(ctx: PricingContext): RuleResult & { cleaningFee: number } {
    ...
    // Extra patients (> 2 patients)
    const extrapatients = Math.max(0, ctx.patientCount - 2);
    fee += extrapatients * CLEANING_FEE_PER_patient;
    ...
    return {
      ...rulePassed(
        ...
        `Cleaning fee: ¥${fee} (${nights} nights, ${ctx.patientCount} patients${extrapatients > 0 ? `, +${extrapatients} extra` : ""}${nights > LONG_STAY_THRESHOLD_NIGHTS ? `, long-stay discount applied` : ""})`
      ),
      cleaningFee: fee,
    };
  }
