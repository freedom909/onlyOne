...
const mockRepo = {
  ...
  delete: jest.fn(),
  findByLatestByPatientId: jest.fn(),
};
...
describe("CreateBookingUseCase", () => {
  ...
  const actor = { patientId: "patient-1" };
  ...
  it("should throw if patientId is missing from actor", async () => {
    await expect(
      useCase.execute({
        listingId: "listing-1",
        checkInDate: "2024-06-01",
    ...
    ).rejects.toThrow("Missing required authentication: patientId");
  });
