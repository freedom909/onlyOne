...
describe("Review Resolvers", () => {
  ...
  beforeEach(() => {
    ...
    (container.resolve as jest.Mock).mockImplementation((token: symbol) => {
      ...
      if (token === Symbol.for("SubmitPatientReviewUseCase")) return mockSubmitUseCase;
      if (token === Symbol.for("SubmitOwnerReplyToPatientReviewUseCase")) return mockOwnerReplyUseCase;
  ...
  describe("Mutation.submitPatientReview", () => {
    it("should submit a patient review with user ID from context", async () => {
      ...
      const context = { user: { id: "patient-1", userId: "patient-1" } };

      const result = await (reviewResolvers as any).Mutation.submitPatientReview(
        null,
        { input },
        context
      );
  ...
  describe("Mutation.submitOwnerReplyToPatientReview", () => {
    it("should submit owner reply with user ID from context", async () => {
      ...
      const result = await (reviewResolvers as any).Mutation.submitOwnerReplyToPatientReview(
        null,
        { input },
        context
      );
  ...
  describe("Mutation.updateReview", () => {
    it("should update a review", async () => {
      ...
      mockRepo.findById.mockResolvedValue({ id: "review-1", patientId: "patient-1" });
  ...
  describe("Review.author", () => {
    it("should return Patient reference with patientId", () => {
      const review = { patientId: "cust-1" };
      const result = (reviewResolvers as any).Review.author(review);

      expect(result).toEqual({ __typename: "Patient", id: "cust-1" });
    });
  });
