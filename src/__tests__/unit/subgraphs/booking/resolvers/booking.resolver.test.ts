...
describe("Booking Resolvers", () => {
  ...
  describe("cancelBooking resolver", () => {
    it("should call cancelBookingUseCase with correct args", async () => {
      ...
      const context = {
        user: { userId: "patient-1", role: "CUSTOMER" },
      };
  ...
  describe("myBookings query", () => {
    it("should return patient's own bookings", async () => {
      const mockBookings = [
        { id: "booking-1", patientId: "patient-1" },
        { id: "booking-2", patientId: "patient-1" },
      ];
      (requireAuth as jest.Mock).mockResolvedValue({
        userId: "patient-1",
        role: "CUSTOMER",
      });
      mockBookingRepo.findByPatientId.mockResolvedValue(mockBookings);
    ...
    it("should return host's listings bookings + own bookings", async () => {
      const ownerBookings = [{ id: "booking-1", patientId: "guest-1" }];
      const patientBookings = [{ id: "booking-2", patientId: "host-1" }];
      (requireAuth as jest.Mock).mockResolvedValue({
        userId: "host-1",
        role: "DOCTOR",
      });
      ...
      mockBookingRepo.findByPatientId.mockResolvedValue(patientBookings);

      const result = await (resolvers as any).Query.myBookings(
        null,
        {},
        { user: { userId: "host-1", role: "DOCTOR" } }
      );
    ...
    it("should deduplicate bookings when host is also a patient", async () => {
      const sharedBooking = { id: "booking-1", patientId: "host-1" };
      const ownerBookings = [sharedBooking];
      const patientBookings = [sharedBooking, { id: "booking-2", patientId: "host-1" }];
      (requireAuth as jest.Mock).mockResolvedValue({
        userId: "host-1",
        role: "DOCTOR",
      });
