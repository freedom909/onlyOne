...
describe("User Resolvers", () => {
  ...
  describe("Patient.__resolveReference", () => {
    it("should resolve patient by ID", async () => {
      const mockPatient = { id: "cust-1", email: "cust@example.com" };
      mockUserService.findById.mockResolvedValue(mockPatient);
      const ctxContainer = createMockContainer(mockUserService, mockUseCases);

      const result = await (resolvers as any).Patient.__resolveReference(
        { id: "cust-1" },
  ...
  describe("Mutation.becomeHost", () => {
    it("should call becomeHostUseCase.execute with correct args", async () => {
      const mockResult = { id: "user-1", role: "DOCTOR" };
