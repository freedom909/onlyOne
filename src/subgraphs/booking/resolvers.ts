...
export const resolvers = {
  Query: {
    ...
    bookingsForPatient: async (_: any, { userId }: any) => {
      const repo =
    ...
    myBookings: async (_: any, __: any, context: any) => {
      ...
      // OWNER/DOCTOR: return bookings for their listings + their own bookings as patient
      if (role === Role.OWNER || role === Role.DOCTOR) {
        ...
        // Also get bookings where user is the patient (e.g., they booked someone else's listing)
        const patientBookings = await bookingRepo.findByPatientId(userId);

        // Merge and deduplicate by booking ID
        const bookingMap = new Map<string, any>();
        for (const b of [...ownerBookings, ...patientBookings]) {
      ...
      return bookingRepo.findByPatientId(userId);
    },
  },
  Mutation: {
  ...
  async (_: any, { input }: any, context: any) => {
    ...
    cancelBooking: withAuthorization(Action.CANCEL, Resource.BOOKING, async (_: any, { id, reason }: any) => {
      ...
      resolveOwnerId: async (_ctx, { id }) => {
        ...
        // Return the patient (booking creator) — the RBAC policy also allows DOCTOR role via hasMinRole check
