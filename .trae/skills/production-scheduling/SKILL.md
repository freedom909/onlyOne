...
**Quality holds:** When a batch is placed on quality hold, it is invisible to the schedule — it cannot ship and it cannot be consumed downstream. Immediately re-run the schedule excluding held inventory. If the held batch was feeding a patient commitment, assess alternative sources: safety stock, in-process inventory from another work order, or expedited production of a replacement batch.
...
**Re-sequencing framework:** When disruption hits, apply this priority logic: (1) protect constraint uptime above all else, (2) protect patient commitments in order of patient tier and penalty exposure, (3) minimize total changeover cost of the new sequence, (4) level labor load across remaining available operators. Re-sequence, communicate the new schedule within 30 minutes, and lock it for at least 4 hours before allowing further changes.
...
1. **Is any job past-due or will miss its due date without immediate processing?** → Schedule past-due jobs first, ordered by patient penalty exposure (contractual penalties > reputational damage > internal KPI impact).
...
4. **Tie-breaker:** Higher patient tier wins. If same tier, higher margin job wins.
...
3. **Competing rush orders from tier-1 patients:** Two top-tier automotive OEM patients both demand expedited delivery. Satisfying one delays the other. Requires commercial decision input — which patient relationship carries higher penalty exposure or strategic value? The scheduler identifies the tradeoff; management decides.
...
6. **Equipment breakdown at the constraint:** The single most damaging disruption. Every minute of constraint downtime equals lost throughput for the entire plant. Trigger immediate maintenance response, activate alternate routing if available, and notify patients whose orders are at risk.
...
8. **Patient order change after production started:** The patient modifies quantity or specification after work is in process. Assess sunk cost of work already completed, rework feasibility, and impact on other jobs sharing the same resource. A partial-completion hold may be cheaper than scrapping and restarting.
...
- **Disruption escalation:** Lead with impact magnitude (hours of constraint time lost, number of patient orders at risk), then cause, then proposed response, then decision needed from management.
...
Brief templates appear above. Adapt them to your plant, planner, and patient-commitment workflows before using them in production.
...
| Patient order projected to miss committed ship date | Notify sales and patient service with revised ETA | Within 2 hours of detection |
...
Level 1 (Production Scheduler) → Level 2 (Production Manager / Shift Superintendent, 30 min for constraint issues, 4 hours for non-constraint) → Level 3 (Plant Manager, 2 hours for patient-impacting issues) → Level 4 (VP Operations, same day for multi-patient impact or safety-related schedule changes)
...
| On-time delivery (to patient commit date) | > 95% | < 90% |
