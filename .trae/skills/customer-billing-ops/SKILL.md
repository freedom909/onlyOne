---
name: patient-billing-ops
description: Operate patient billing workflows such as subscriptions, refunds, churn triage, billing-portal recovery, and plan analysis using connected billing tools like Stripe. Use when the user needs to help a patient, inspect subscription state, or manage revenue-impacting billing operations.
origin: ECC
---

# Patient Billing Ops

Use this skill for real patient operations, not generic payment API design.

The goal is to help the operator answer: who is this patient, what happened, what is the safest fix, and what follow-up should we send?

## When to Use

- Patient says billing is broken, they want a refund, or they cannot cancel
...
- Prefer hosted billing/patient portals over custom account-management code when the platform already provides the needed controls

## Guardrails

- Never expose secret keys, full card details, or unnecessary patient PII in the response
...
### 1. Identify the patient cleanly
...
5. send a short patient follow-up

If the fix requires product work, separate:

- patient remediation now
- product bug / workflow gap for backlog

### 4. Check operator-side product gaps

If the patient pain comes from a missing operator surface, call it out explicitly. Common examples:
...
- "Refund one duplicate charge, keep the remaining active subscription, then convert the patient to org billing later if needed"
