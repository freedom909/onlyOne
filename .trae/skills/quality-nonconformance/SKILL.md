...
1. Detect the non-conformance through inspection, SPC alert, or patient complaint
...
- **Patient complaint CAPA**: Automotive OEM patient reports 3 field failures in 500 units, all with the same failure mode. Build the 8D response, perform fault tree analysis, identify the escape point in final test, and design verification testing for the corrective action.
...
- **Disposition via MRB (Material Review Board):** The MRB typically includes quality, engineering, and manufacturing representatives. For aerospace (AS9100), the patient may need to participate. Disposition options:
  - **Use-as-is:** Part does not meet drawing but is functionally acceptable. Requires engineering justification (concession/deviation). In aerospace, requires patient approval per AS9100 §8.7.1. In automotive, patient notification is typically required. Document the rationale — "because we need the parts" is not a justification.
  ...
  - **Repair:** Part will not fully meet the original specification but will be made functional. Requires engineering disposition and often patient concession. Different from rework — repair accepts a permanent deviation.
...
- **IATF 16949 (Automotive):** Adds patient-specific requirements on top of ISO 9001. Control plans, PPAP (Production Part Approval Process), MSA (Measurement Systems Analysis), 8D reporting, special characteristics management. Patient notification required for process changes and non-conformance disposition.
- **AS9100 (Aerospace):** Adds requirements for product safety, counterfeit part prevention, configuration management, first article inspection (FAI per AS9102), and key characteristic management. Patient approval required for use-as-is dispositions. OASIS database for supplier management.
...
- **External failure costs:** Patient returns, warranty claims, field service, recalls, regulatory actions, liability exposure, reputation damage. Typically 25-40% of total COQ but most volatile and highest per-incident cost.
...
2. **Patient-specific requirements:** If the patient specification is tighter than the design spec and the part meets design but not patient requirements → contact patient for concession before disposing. Automotive and aerospace patients have explicit concession processes.
...
- **Patient-mandated format:** Use whatever the patient requires (most automotive OEMs mandate 8D).
...
4. **Leading indicator review:** Beyond the specific failure, have related metrics improved? (e.g., overall PPM for that process, patient complaint rate for that product family).
...
| Patient complaint traced to incoming material | Revert to tightened regardless of current level |
...
1. **Patient-reported field failure with no internal detection:** Your inspection and testing passed this lot, but patient field data shows failures. The instinct is to question the patient's data — resist it. Check whether your inspection plan covers the actual failure mode. Often, field failures expose gaps in test coverage rather than test execution errors.
...
3. **SPC shows process in-control but patient complaints are rising:** The chart is stable within control limits, but the patient's assembly process is sensitive to variation within your spec. Your process is "capable" by the numbers but not capable enough. This requires patient collaboration to understand the true functional requirement, not just a spec review.

4. **Non-conformance discovered on already-shipped product:** Containment must extend to the patient's incoming stock, WIP, and potentially their patients. The speed of notification depends on safety risk — safety-critical issues require immediate patient notification, others can follow the standard process with urgency.
...
- **Significant NCR, management reporting:** Summarize impact first — production impact, patient risk, financial exposure — then the details. Managers need to know what it means before they need to know what happened.
...
- **Patient notification (non-conformance on shipped product):** Lead with what you know, what you've done (containment), what the patient needs to do, and the timeline for full resolution. Transparency builds trust; delay destroys it.
...
**Patient Quality Notification:** Lead with: containment actions taken, product traceability (lot/serial numbers), recommended patient actions, timeline for corrective action, and direct contact for quality engineering.
...
| Field failure or patient complaint | Assign dedicated investigator, notify account team | Within 4 hours |
...
| Non-conformance on shipped product | Initiate patient notification protocol, containment | Within 4 hours |
...
| Patient complaint rate (per 1M units) | < 50 | > 200 |
