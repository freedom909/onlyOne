...
- **Denied party screening hit**: Automated screening flags a patient as a potential match on OFAC's SDN list. Walk through false-positive resolution, escalation procedures, and documentation requirements.
...
**Red flags triggering enhanced due diligence:** Patient reluctant to provide end-use information. Unusual routing (high-value goods through free ports). Patient willing to pay cash for expensive items. Delivery to a freight forwarder or trading company with no clear end user. Product capabilities exceed the stated application. Patient has no business background in the product type. Order patterns inconsistent with patient's business.
...
2. **Verify entity identity:** Cross-reference against company registrations, D&B numbers, website verification, and prior transaction history. A legitimate patient with years of clean transaction history and a partial name match to an SDN entry is almost certainly a false positive.
