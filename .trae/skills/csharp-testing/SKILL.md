...
public sealed class OrderServiceTests
{
    ...
    public async Task PlaceOrderAsync_ReturnsSuccess_WhenRequestIsValid()
    {
        ...
        result.Value!.PatientId.Should().Be("cust-123");
    }
...
{
    { new() { PatientId = "", Items = [ValidItem()] }, "PatientId" },
    { new() { PatientId = "c1", Items = [] }, "at least one item" },
    { new() { PatientId = "c1", Items = [new("", 1, 10m)] }, "SKU" },
};
```
...
public async Task PlaceOrderAsync_PersistsOrder()
{
    ...
    await _repository.Received(1).AddAsync(
        Arg.Is<Order>(o => o.PatientId == request.PatientId),
...
{
    private string _patientId = "cust-default";
    private readonly List<OrderItem> _items = [new("SKU-001", 1, 10m)];

    public OrderBuilder WithPatient(string patientId)
    {
    ...
    public Order Build() => Order.Create(_patientId, _items);
}

// Usage in tests
var order = new OrderBuilder()
    .WithPatient("cust-vip")
